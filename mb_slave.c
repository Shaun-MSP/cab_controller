   ...
   size_t rx_count)
{
   ...
   write_address = request->write_address;
   write_quantity = request->write_quantity;
   ...
   read_address = request->read_address;
   read_quantity = request->read_quantity;
...
void mb_slave_handle_request (mb_slave_t * slave, pdu_txn_t * transaction)
{
   ...
   if (rx_count > 0)
   {
      C_SerialPrint("rx_count: ");
      C_SerialPrintnum(rx_count);
