...
static int mb_tcp_shutdown (mb_transport_t * transport, int arg)
{
   ...
   C_SerialPrintln("mb_tcp_shutdown");
   ...
   size_t size)
{
   ...
   mbap->id       = transaction->id;
   mbap->length   = (uint16_t)size + 1; /* Includes size of unit id */
   mbap->protocol = 0;
   ...
   uint32_t tmo)
{
  ...
  if (result > 0)
   {
     C_SerialPrint("Bytes received: ");
     C_SerialPrintnum(result);
   }
